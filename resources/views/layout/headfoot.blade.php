...
<body class="bg-gray-100 text-gray-800">
    <!-- Navbar -->
    <nav class="navbar fixed top-0 w-full z-50 bg-gradient-to-br from-primary-500 to-primary-600 shadow-lg transition-all duration-300 backdrop-blur-md"
         :class="{ 'scrolled': scrolled }" x-data="{ isMobileMenuOpen: false, scrolled: false }" 
         @scroll.window="scrolled = window.scrollY > 50">
        <div class="nav-container max-w-7xl mx-auto px-8 h-20 flex justify-between items-center transition-all duration-300 gap-16"
             :class="{ 'h-16': scrolled }">
            <!-- Logo Section -->
            ...
            @auth
                <div class="hidden md:flex items-center space-x-4">
                    ...
                    <form action="{{ route('logout') }}" method="POST" class="inline">
                        @csrf
                        <button type="submit" class="ml-2 px-3 py-1 rounded bg-white bg-opacity-20 text-white text-sm hover:bg-opacity-30 transition">Logout</button>
                    </form>
                </div>
            @endauth
